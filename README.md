...
| Platform | Cordova platform versions | Build compatibility | Runtime behavior notes |
| --- | --- | --- | --- |
| Android | `cordova-android@14.x`, `cordova-android@15.x` | Supported | On Android API 36+, large-screen and foldable devices may ignore orientation locks. A successful plugin call means the request was accepted by Android, not that visible rotation is guaranteed. |
| iOS | `cordova-ios@7.x`, `cordova-ios@8.x` | Supported | Orientation lock and unlock are supported; iOS enforces system-level orientation policies that may limit visible rotation in some contexts. |
